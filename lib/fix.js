        ...
        switch (token) {
            case '"':
                ...
                switch (peekPrevResult.lastChar) {
                    ...
                    case undefined:
                        iterator.set('"');
                        // record old index
                        const index0 = iterator.index;
                        // find next '"'
                        skipString(iterator);
                        // new index
                        const index1 = iterator.index;
                        if (index0 === index1) {
                            // not found
                            iterator.set('');
                        } else if (!iterator.done()) {
                            // found, delete '"' and append '"' to last char
                            iterator.array[index1 - 1] += '"';
                            iterator.set('');
                        } else {
                            // end of iterator
                            if (iterator.get() !== '"') {
                                iterator.array[iterator.array.length - 1] = iterator.array[iterator.array.length - 1] + '"';
                            }
                        }
                        break;
                    default:
            ...
            case '[':
                ...
                switch (peekPrevResult.lastChar) {
                    ...
                    case ':':
                    case undefined:
                        stack.push({ index: iterator.index, value: token});
                        break;
                    default:
                        iterator.set('');
                }
